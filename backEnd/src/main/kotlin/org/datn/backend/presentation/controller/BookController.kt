...
class BookController(
    ...
    ): ResponseEntity<Long> = ResponseEntity.ok(bookService.getCategoryStats(categoryId))
    ...
    fun getAll(pageable: Pageable): ResponseEntity<BookPageResponse> = ResponseEntity.ok(bookService.getAll(pageable).toPageResponse())
    ...
    fun search(
        ...
        pageable: Pageable,
    ): ResponseEntity<BookPageResponse> = ResponseEntity.ok(bookService.search(query, pageable).toPageResponse())

    @GetMapping("/author/{authorId}", produces = ["application/x-protobuf"])
    fun getBooksByAuthor(
        @PathVariable authorId: Long,
        pageable: Pageable,
    ): ResponseEntity<BookPageResponse> = ResponseEntity.ok(bookService.getBooksByAuthor(authorId, pageable).toPageResponse())

    @PostMapping(consumes = ["application/x-protobuf"], produces = ["application/x-protobuf"])
    fun create(
        @RequestBody bookProto: BookProto,
    ): ResponseEntity<BookProto> {
