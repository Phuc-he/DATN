...
class MessageController(
    ...
    fun create(@RequestBody request: MessageResponseProto): ResponseEntity<MessageResponseProto> {
        val userEntity = if (request.hasUser()) {
            userService.getById(request.user.id)
        } else null

        // 2. Tìm Book thật từ DB (tương tự User)
        val bookEntity = if (request.hasRelatedBook()) {
    ...
    fun update(
        ...
        return ResponseEntity.ok(messageService.update(id, updates)?.toProto())
    }
    ...
    suspend fun chat(@RequestBody request: MessageResponseProto): ResponseEntity<MessageResponseProto> {
        val managedUser = request.user?.id?.let { userId ->
            userService.getById(userId)
        }
        return ResponseEntity.ok(
            messageService.chatWithAI(
                Message(
                    user = managedUser,
    ...
    suspend fun train(): ResponseEntity<Unit> {
        ...
        messageService.trainAiModel(buildList {
            // Kiến thức về Tác giả
            addAll(authorService.getAll().map {
//                val text = "Tác giả: ${it.name}, Tiểu sử: ${it.bio}"
                """{"instruction": "Khi khách hàng hỏi về tác giả ${it.name}", "input": "Tác giả này là ai?", "output": "Đây là ${it.name}. ${it.bio}"}""".replace(
                    "\n",
                    ""
                )
            })
            ...
            // Kiến thức về Sách
            addAll(bookService.getAll().map {
//                val text = "Sách: ${it.title}, Giá: ${it.price}, Mô tả: ${it.description}"
                """{"instruction": "Thông tin chi tiết về sách ${it.title}", "input": "Cuốn sách ${it.title} có gì đặc biệt?", "output": "Cuốn sách này có giá ${it.price}. Mô tả: ${it.description}"}""".replace(
                    "\n",
                    ""
                )
            })
            ...
            addAll(voucherService.getAll().map {
//                val text = "Mã giảm giá: ${it.code}, Giảm: ${it.discountValue}, Đơn tối thiểu: ${it.minOrderValue}"
                """{"instruction": "Kiểm tra mã giảm giá ${it.code}", "input": "Tôi có mã ${it.code}, nó dùng như thế nào?", "output": "Mã ${it.code} là mã giảm giá trị giá ${it.discountValue}, áp dụng cho đơn hàng tối thiểu từ ${it.minOrderValue}."}""".replace(
                    "\n",
                    ""
                )
            })
    ...
    fun search(@RequestParam query: String, pageable: Pageable): ResponseEntity<MessageResponsePageResponse> =
        ResponseEntity.ok(messageService.search(query, pageable).toPageResponse())
    ...
    fun getChatHistory(@PathVariable userId: Long): ResponseEntity<MessageResponseProtoList> = ResponseEntity.ok(
        MessageResponseProtoList.newBuilder()
