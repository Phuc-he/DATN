...
class CategoryController(
    ...
    fun search(
        ...
        pageable: Pageable,
    ): ResponseEntity<CategoryPageResponse> = ResponseEntity.ok(categoryService.search(query, pageable).toPageResponse())
    ...
    ): ResponseEntity<CategoryProto> = ResponseEntity.ok(categoryService.getById(id).toProto())

    @PostMapping(consumes = ["application/x-protobuf"], produces = ["application/x-protobuf"])
    fun create(
        @RequestBody proto: CategoryProto,
