...
class OrderItemController(
    ...
    ): ResponseEntity<List<OrderItemProto>> = ResponseEntity.ok(orderItemService.getItemsByOrderId(orderId).map { it.toProto() })

    @GetMapping("/{id}", produces = ["application/x-protobuf"])
    fun getById(
        @PathVariable id: Long,
    ): ResponseEntity<OrderItemProto> = ResponseEntity.ok(orderItemService.getById(id).toProto())
