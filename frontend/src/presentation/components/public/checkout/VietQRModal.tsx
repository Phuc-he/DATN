...
export const VietQRModal: React.FC<VietQRProps> = ({ qrUrl, amount, orderId }) => {
  return (
    <div className="bg-white p-8 rounded-3xl shadow-xl max-w-md mx-auto text-center border border-slate-100">
      <h3 className="text-2xl font-black text-slate-950 mb-2">Thanh toán qua VietQR</h3>
      <p className="text-emerald-800 text-sm mb-6">Sử dụng ứng dụng VCB Digibank hoặc Ngân hàng bất kỳ để quét mã</p>
      
      <div className="relative aspect-square w-64 mx-auto mb-6 border-4 border-emerald-50 rounded-2xl p-2">
      ...
      <div className="bg-emerald-50 rounded-2xl p-4 mb-6 text-left space-y-2">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="font-bold text-emerald-600">DH{orderId}</span>
        </div>
      </div>
