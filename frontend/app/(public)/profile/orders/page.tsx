...
const OrderStatusBadge = ({ status }: { status: OrderStatus }) => {
  ...
  const colorClasses: Record<number, string> = {
    ...
    [OrderStatus.PROCESSING]: "bg-emerald-50 text-emerald-700 border-emerald-100",
    [OrderStatus.PAID]: "bg-emerald-50 text-emerald-700 border-emerald-100",
    [OrderStatus.SHIPPED]: "bg-emerald-50 text-emerald-700 border-emerald-100",
  ...
  if (authLoading || loading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[60vh] gap-4">
        ...
        <p className="text-emerald-900 font-medium">Đang tải lịch sử đơn hàng...</p>
      </div>
    );
  }
    ...
    <div className="bg-emerald-50 min-h-screen py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl font-black text-slate-950 tracking-tight">Lịch sử đơn hàng</h1>
            <p className="text-emerald-900 mt-1">Theo dõi quá trình vận chuyển và quản lý các giao dịch</p>
          </div>
          ...
          <div className="relative flex flex-row items-center w-full md:w-80 group">
            ...
            <input
              ...
              className="w-full p-2 h-11 pl-11 pr-10 bg-white border-2 border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 transition-all text-sm font-bold text-slate-950 shadow-sm"
            />
            {searchQuery && (
              <button
                type="button"
            ...
            {filteredOrders.map((order) => (
              <div key={order.id} className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div className="p-6">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-emerald-50">
                  ...
                  <div className="flex flex-col lg:flex-row justify-between gap-8">
                    ...
                    <div className="lg:w-64 bg-emerald-50 rounded-2xl p-5 flex flex-col justify-between">
                      <div>
                      ...
                      <Link
                        ...
                        className="mt-4 w-full flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-950 py-2.5 rounded-xl font-bold hover:bg-slate-950 hover:text-white hover:border-slate-950 transition-all"
                      >
